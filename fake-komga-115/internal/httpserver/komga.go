...
import (
	...
	"fake-komga-local/internal/buildinfo"
...
func (s *Server) komgaSeriesByID(w http.ResponseWriter, r *http.Request) {
	...
	meta, _ := s.store.SeriesMetaByID(r.Context(), item.ID)
	writeJSON(w, 200, seriesDTOWithProgress(item, progress, meta))
}
...
func (s *Server) komgaBookByID(w http.ResponseWriter, r *http.Request) {
	...
	bookMeta, _ := s.store.BookMetaByID(r.Context(), book.ID)
	writeJSON(w, 200, bookDTOWithProgress(book, series, progress, ok, bookMeta))
}
...
func (s *Server) seriesDTOs(r *http.Request, items []database.Series) ([]any, error) {
	...
	for _, item := range items {
		...
		meta, _ := s.store.SeriesMetaByID(r.Context(), item.ID)
		out = append(out, seriesDTOWithProgress(item, progress, meta))
	}
	return out, nil
}
...
func (s *Server) bookDTOs(r *http.Request, items []database.Book) ([]any, error) {
	...
	for _, item := range items {
		...
		bookMeta, _ := s.store.BookMetaByID(r.Context(), item.ID)
		out = append(out, bookDTOWithProgress(item, series, progress, ok, bookMeta))
	}
	return out, nil
}
...
func (s *Server) komgaBookSibling(w http.ResponseWriter, r *http.Request, next bool) {
	...
	bookMeta, _ := s.store.BookMetaByID(r.Context(), sibling.ID)
	writeJSON(w, http.StatusOK, bookDTOWithProgress(sibling, series, progress, ok, bookMeta))
}
...
func seriesDTO(item database.Series) map[string]any {
	...
	return seriesDTOWithProgress(item, progress, nil)
}

func seriesDTOWithProgress(
	item database.Series,
	progress database.SeriesReadProgress,
	meta *database.BangumiSeriesMeta,
) map[string]any {
	...
	if meta != nil {
		...
		if meta.TitleJP != "" {
			altTitles = append(altTitles, map[string]any{"label": "原名", "title": meta.TitleJP})
		}
		if meta.Summary != "" {
		...
		if meta.Platform != "" {
			genres = append(genres, meta.Platform)
		}
		if meta.TotalVolumes > 0 {
			totalBookCount = meta.TotalVolumes
		}
		if meta.TagsJSON != "" && meta.TagsJSON != "[]" {
			json.Unmarshal([]byte(meta.TagsJSON), &tags)
		}
		if meta.AuthorsJSON != "" && meta.AuthorsJSON != "[]" {
			var rawAuthors []struct {
				Name string `json:"name"`
				Role string `json:"role"`
			}
			if err := json.Unmarshal([]byte(meta.AuthorsJSON), &rawAuthors); err == nil {
				for _, a := range rawAuthors {
					authors = append(authors, map[string]any{"name": a.Name, "role": a.Role})
				}
			}
		}
		if meta.BangumiID > 0 {
			links = append(links, map[string]any{
				"label": "bangumi",
				"url":   fmt.Sprintf("https://bangumi.tv/subject/%d", meta.BangumiID),
			})
		}
	}
	...
	return map[string]any{
		...
		"metadata": map[string]any{
			"status": status, "statusLock": false,
			"created": created, "lastModified": updated,
			"title": title, "titleLock": false, "titleSort": titleSort, "titleSortLock": false,
			"summary": summary, "summaryLock": false,
			"readingDirection": "LEFT_TO_RIGHT", "readingDirectionLock": false,
			"publisher": publisher, "publisherLock": false, "ageRating": nil, "ageRatingLock": false,
			"language": "", "languageLock": false, "genres": genres, "genresLock": false,
			"tags": tags, "tagsLock": false, "totalBookCount": totalBookCount, "totalBookCountLock": false,
			"sharingLabels": []string{}, "sharingLabelsLock": false,
			"links": links, "linksLock": false, "alternateTitles": altTitles, "alternateTitlesLock": false,
		},
		"booksMetadata": map[string]any{
			"authors": authors, "tags": tags, "releaseDate": nil,
			"summary": summary, "summaryNumber": "", "created": created, "lastModified": updated,
		},
...
func bookDTO(item database.Book, series database.Series) map[string]any {
	return bookDTOWithProgress(item, series, database.BookReadProgress{}, false, nil)
}
...
func bookDTOWithProgress(
	...
	return map[string]any{
		...
		"metadata": map[string]any{
			"title": title, "titleLock": false, "summary": bookSummary, "summaryLock": false,
			"number": number, "numberLock": false, "numberSort": item.NumberSort, "numberSortLock": false,
			"releaseDate": releaseDate, "releaseDateLock": false, "authors": bookAuthors, "authorsLock": false,
			"tags": bookTags, "tagsLock": false, "isbn": isbn, "isbnLock": false,
			"links": bookLinks, "linksLock": false, "created": created, "lastModified": updated,
		},
