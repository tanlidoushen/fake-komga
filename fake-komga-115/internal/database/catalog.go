...
func (s *Store) SeriesMetaByID(ctx context.Context, seriesID string) (*BangumiSeriesMeta, error) {
	var m BangumiSeriesMeta
	err := s.db.QueryRowContext(ctx, `
		...
		FROM bangumi_series_meta WHERE series_id=?`, seriesID).Scan(
		&m.SeriesID, &m.BangumiID, &m.TitleCN, &m.TitleJP, &m.Summary, &m.Publisher, &m.Status,
		&m.TotalVolumes, &m.Rating, &m.RatingCount, &m.TagsJSON, &m.AuthorsJSON, &m.CoverURL, &m.Platform, &m.UpdatedAt)
	if err != nil {
		return nil, err
	}
	return &m, nil
...
func (s *Store) BookMetaByID(ctx context.Context, bookID string) (*BangumiBookMeta, error) {
	var m BangumiBookMeta
	err := s.db.QueryRowContext(ctx, `
		SELECT book_id, series_id, volume_number, isbn, release_date, summary, cover_url, updated_at
		FROM bangumi_book_meta WHERE book_id=?`, bookID).Scan(
		&m.BookID, &m.SeriesID, &m.VolumeNumber, &m.ISBN, &m.ReleaseDate, &m.Summary, &m.CoverURL, &m.UpdatedAt)
	if err != nil {
		return nil, err
	}
	return &m, nil
